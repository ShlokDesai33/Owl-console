...
import InputField from '../../../../../../components/resources/post/fields/input'
...
const PostInputFields = ({ id, resState }: Props) => {
  const [customFields, setCustomFields] = useState<CustomInputField[]>([]);
      ...
      return (
        <>
          ...
          <main className="pt-12 px-12 overflow-y-scroll">
            <form action={`/dashboard/resources/${id}/post/custom/input`} method="post" onSubmit={() => {
              setTimeout(() => {
                setState('loading');
              }, 500);
            }}>
              <div className="border-2 rounded-xl p-10">
                <DynamicHeader param="custom_input" />
  ...
  try {
    ...
    for (let key in body) {
      ...
      if (Array.isArray(val)) {
        ...
        let obj: { title: string, type: string, content?: any[] } = {
          title: val.shift(),
          type: val.pop()
        }
        ...
        while (val.length > 0) {
          const option = val.shift();
          const price = val.shift();

          if (isNaN(price)) continue;

          content.push({ value: option, priceAddition: Number(price) });
        }
        ...
        batch.set(doc(db, `resources/${id}/custom_input`, String(count)), obj);
        count += 1;
      }
    }
